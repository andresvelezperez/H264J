...
public class DSPContext {
    ...
    /**
     * idct input permutation. several optimized IDCTs need a permutated input
     * (relative to the normal order of the reference IDCT) this permutation
     * must be performed before the idct_put/add, note, normally this can be
     * merged with the zigzag/alternate scan<br>
     * an example to avoid confusion: - (->decode coeffs -> zigzag reorder ->
     * dequant -> reference idct ->...) - (x -> referece dct -> reference idct
     * -> x) - (x -> referece dct -> simple_mmx_perm = idct_permutation ->
     * simple_idct_mmx -> x) - (->decode coeffs -> zigzag reorder ->
     * simple_mmx_perm -> dequant -> simple_idct_mmx ->...)
     */
     ...
     * @param linesize number of bytes between 2 vertically adjacent samples in
...
//	public void put_h264_chroma_mc8_c(uint8_t *dst/*align 8*/, uint8_t *src/*align 1*/, int stride, int h, int x, int y){\
    ...
    public static int op_avg(int a, int b, int[] cm_base, int cm_offset) {
        return (((a) + cm_base[cm_offset + (((b) + 16) >> 5)] + 1) >> 1);
    }

    public static int op_put(int a, int b, int[] cm_base, int cm_offset) {
        return cm_base[cm_offset + (((b) + 16) >> 5)];
    }

    public static int op_avg2(int a, int b, int[] cm_base, int cm_offset) {
        return (((a) + cm_base[cm_offset + (((b) + 512) >> 10)] + 1) >> 1);
    }
