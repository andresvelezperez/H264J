...
public class MpegEncContext {
    ...
    /**
     * Size of the frame reordering buffer in the decoder. For MPEG-2 it is 1
     * IPB or 0 low delay IP. - encoding: Set by libavcodec. - decoding: Set by
     * libavcodec.
     */
     ...
     * streams is not written to for at least s->max_b_frames+1 frames, if this
     * is not set the input will be copied.
     */
    ...
    /**
     * Input bitstream might be truncated at a random location instead of only
     * at frame boundaries.
     */
...
//};
     ...
     * bitstream readers read 32 or 64 bit at once and could read over the
     * end.<br>
     * Note: If the first 23 bits of the additional bytes are not 0, then
     * damaged MPEG bitstreams could cause overread and segfault.
     */
    ...
    /**
     * strictly follow the standard (MPEG4, ...). - encoding: Set by user. -
     * decoding: Set by user. Setting this to STRICT or higher means the encoder
     * and decoder will generally do stupid things, whereas setting it to
     * unofficial or lower will mean the encoder might produce output that is
     * not supported by all spec-compliant decoders. Decoders don't
     * differentiate between normal, unofficial and experimental (that is, they
     * always try to decode things when they can) unless they are explicitly
     * asked to behave stupidly (=strictly conform to the specs)
     */
    ...
    /**
     * This defines the location of chroma samples. - encoding: Set by user -
 ...
 /*
     ...
     * 1/framerate and timestamp increments should be identically 1. - encoding:
     * MUST be set by user. - decoding: Set by libavcodec.
     */
    public AVRational time_base;

    /**
     * IDCT algorithm, see FF_IDCT_* below. - encoding: Set by user. - decoding:
     * Set by user.
     */
    ...
    /**
     * MPEG vs JPEG YUV range. - encoding: Set by user - decoding: Set by
     * libavcodec
     */
    /*enum AVColorRange*/ public int color_range;

    /**
     * Chromaticity coordinates of the source primaries. - encoding: Set by user
     * - decoding: Set by libavcodec
     */
    ...
    /**
     * YUV colorspace type. - encoding: Set by user - decoding: Set by
     * libavcodec
     */
    ...
    /**
     * number of reference frames - encoding: Set by user. - decoding: Set by
     * lavc.
     */
    public int refs;
    ...
    /**
     * Pixel format, see PIX_FMT_xxx. May be set by the demuxer if known from
     * headers. May be overriden by the decoder if it knows better. - encoding:
     * Set by user. - decoding: Set by user if known, overridden by libavcodec
     * if known
     */
     ...
     * misdetect some more or less valid parts as errors. - encoding: unused -
    ...
    public int hurry_up;
    /**
     * < when set to 1 during decoding, b frames will be skipped when set to 2
     * idct/dequant will be skipped too
     */
     ...
     *
     * Set to time_base ticks per frame. Default 1, e.g., H.264/MPEG-2 set it to
     * 2.
     */
    ...
    /**
     * low resolution decoding, 1-> 1/2 size, 2->1/4 size - encoding: unused -
     * decoding: Set by user.
     */
    public int lowres;

    /**
     * Bitstream width / height, may be different from width/height if lowres or
     * other things are used. - encoding: unused - decoding: Set by user before
     * init if known. Codec should override / dynamically change if needed.
     */
    ...
    /**
     * the picture in the bitstream - encoding: Set by libavcodec. - decoding:
     * Set by libavcodec.
     */
    ...
    /**
     * copy of the current picture structure. note, linesize & data, might not
     * match the current picture (for field pictures)
     */
    public AVFrame current_picture = new AVFrame();    ///< buffer to store the decompressed current picture

    /**
     * internal_buffer count Don't touch, used by libavcodec
     * default_get_buffer().
     */
    public int internal_buffer_count;

    /**
     * copy of the previous picture structure. note, linesize & data, might not
     * match the previous picture (for field pictures)
     */
    public AVFrame last_picture = new AVFrame();

    /**
     * copy of the next picture structure. note, linesize & data, might not
     * match the next picture (for field pictures)
     */
     ...
     * packet into every function. Currently only valid inside lavc and
     * get/release_buffer callbacks. - decoding: set by avcodec_decode_*, read
     * by get_buffer() for setting pkt_pts - encoding: unused
     */
    public AVPacket pkt;
    ...
    /**
     * opaque 64bit number (generally a PTS) that will be reordered and output
     * in AVFrame.reordered_opaque
     *
     * @deprecated in favor of pkt_pts - encoding: unused - decoding: Set by
     * user.
     */
    ...
    /**
     * some codecs need / can use extradata like Huffman tables. mjpeg: Huffman
     * tables rv10: additional flags mpeg4: global headers (they can be in the
     * bitstream or here) The allocated memory should be
     * FF_INPUT_BUFFER_PADDING_SIZE bytes larger than extradata_size to avoid
     * prolems if it is read with the bitstream reader. The bytewise contents of
     * extradata must not depend on the architecture or CPU endianness. -
     * encoding: Set/allocated/freed by libavcodec. - decoding:
     * Set/allocated/freed by user.
     */
    /*uint8_t **/
    ...
    /**
     * allocates a Picture The pixels are allocated/set by calling get_buffer()
     * if shared=0
     */
    ...
    /**
     * generic function for encode/decode called after coding/decoding the
     * header and before a frame is coded/decoded
     */
...
//        if(this.flags && CODEC_FLAG_QSCALE)
 ...
 /* Only suport H264
        ...
        if (this.last_picture_ptr != null) {
            ff_copy_picture(this.last_picture, this.last_picture_ptr);
        }
        if (this.next_picture_ptr != null) {
            ff_copy_picture(this.next_picture, this.next_picture_ptr);
        }
 ...
 /*??????????????
    ...
    /**
     * init common structure for both encoder and decoder. this assumes that
     * some variables like width/height are already set
     */
...
//	    memset(h, 0, sizeof(H264Context));
    ...
    /**
     * sets the given MpegEncContext to common defaults (same for encoding and
     * decoding). the changed fields will not depend upon the prior state of the
     * MpegEncContext.
     */
