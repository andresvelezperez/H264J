...
public class ErrorResilience {
    ...
    public static void guess_dc(MpegEncContext s, int[] dc_base, int dc_offset,
        ...
        for (b_y = 0; b_y < h; b_y++) {
            for (b_x = 0; b_x < w; b_x++) {
                ...
                if ((((s.current_picture.mb_type_base[s.current_picture.mb_type_offset + mb_index])) & (H264Context.MB_TYPE_16x16 | H264Context.MB_TYPE_16x8 | H264Context.MB_TYPE_8x16 | H264Context.MB_TYPE_8x8)) != 0) {
                    continue; //inter
                }
                if (0 == (error & MpegEncContext.DC_ERROR)) {
                    continue;           //dc-ok
                }
                /* right block */
                for (j = b_x + 1; j < w; j++) {
        ...
        for (b_y = 0; b_y < h; b_y++) {
            for (b_x = 0; b_x < w - 1; b_x++) {
                ...
                if ((0 == left_intra) && (0 == right_intra)
                        && Math.abs(left_mv[0] - right_mv[0]) + Math.abs(left_mv[1] + right_mv[1]) < 2) {
                    continue;
                }
        ...
        for (b_y = 0; b_y < h - 1; b_y++) {
            for (b_x = 0; b_x < w; b_x++) {
                ...
                if ((0 == top_intra) && (0 == bottom_intra)
                        && Math.abs(top_mv[0] - bottom_mv[0]) + Math.abs(top_mv[1] + bottom_mv[1]) < 2) {
                    continue;
                }
    ...
    public static void guess_mv(MpegEncContext s) {
        ...
        for (i = 0; i < s.mb_num; i++) {
            ...
            if (0 == (error & MpegEncContext.MV_ERROR)) {
        ...
        if ((0 == (error_concealment & FF_EC_GUESS_MVS)) || num_avail <= mb_width / 2) {
            for (mb_y = 0; mb_y < s.mb_height; mb_y++) {
                for (mb_x = 0; mb_x < s.mb_width; mb_x++) {
                    ...
                    if (0 == (s.error_status_table[mb_xy] & MpegEncContext.MV_ERROR)) {
                        continue;
                    }
...
//	        printf(":"); fflush(stdout);
     ...
     * the previous line
     * @param status the status at the end (MV_END, AC_ERROR, ...), it is
    ...
    public static void ff_er_frame_end(MpegEncContext s) {
        ...
        if (0 == s.error_recognition || s.error_count == 0 || 0 != s.lowres
                ...
                || // we dont support ER of field pictures yet, though it should not crash if enabled
                s.error_count == 3 * s.mb_width * (skip_top + skip_bottom)) {
            return;
        }
        ...
        for (i = 0; i < s.mb_num; i++) {
            ...
            if (0 != (error & MpegEncContext.AC_ERROR)) {
                ac_error++;
            }
            if (0 != (error & MpegEncContext.MV_ERROR)) {
                mv_error++;
        ...
        if (s.pict_type == H264Context.FF_B_TYPE) {
            for (mb_y = 0; mb_y < s.mb_height; mb_y++) {
                for (mb_x = 0; mb_x < s.mb_width; mb_x++) {
                    ...
                    if (null == s.last_picture.data_base[0]) {
                        s.mv_dir &= ~MpegEncContext.MV_DIR_FORWARD;
                    }
                    if (null == s.next_picture.data_base[0]) {
        ...
        for (mb_y = 0; mb_y < s.mb_height; mb_y++) {
            for (mb_x = 0; mb_x < s.mb_width; mb_x++) {
                ...
                if (0 != (7 & (int) (mb_type)) && 0 != s.partitioned_frame) {
                    continue;
                }
