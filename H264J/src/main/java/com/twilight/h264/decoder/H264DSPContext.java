...
public class H264DSPContext {
        ...
        for (i = 0; i < 4; i++) {
            ...
            for (d = 0; d < 4; d++) {
                ...
                if (Math.abs(p0 - q0) < alpha
                    ...
                    pix[pix_offset - xstride] = av_clip_uint8(p0 + i_delta);
                    /* p0' */
                    pix[pix_offset] = av_clip_uint8(q0 - i_delta);
                    /* q0' */
                }
                pix_offset += ystride;
            }
        ...
        for (i = 0; i < 4; i++) {
            ...
            for (d = 0; d < 2; d++) {
                ...
                if (Math.abs(p0 - q0) < alpha
                    ...
                    pix[pix_offset - xstride] = av_clip_uint8(p0 + delta);
                    /* p0' */
                    pix[pix_offset] = av_clip_uint8(q0 - delta);
                    /* q0' */
                }
                pix_offset += ystride;
            }
    ...
    private void h264_loop_filter_chroma_intra_c(int[] pix, int _pix_offset, int xstride, int ystride, int alpha, int beta) {
        ...
        for (d = 0; d < 8; d++) {
            ...
            if (Math.abs(p0 - q0) < alpha
                ...
                pix[pix_offset - xstride] = (2 * p1 + p0 + q1 + 2) >> 2;
                /* p0' */
                pix[pix_offset] = (2 * q1 + q0 + p1 + 2) >> 2;
                /* q0' */
            }
            pix_offset += ystride;
        }
        ...
        for (i = 0; i < 16; i++) {
            ...
            if (nnz != 0) {
                if (nnz == 1 && block[i * 16] != 0) {
                    h264_idct_dc_add(dst, dst_offset + blockoffset[blockoffset_offset + i], block, block_offset + i * 16, stride);
                } else {
                    idct_internal(dst, dst_offset + blockoffset[blockoffset_offset + i], block, block_offset + i * 16, stride, 4, 6, 1);
                }
            }
        }
    }
    ...
    public void h264_idct8_add4(int[] dst/*align 16*/, int dst_offset, int[] blockoffset, int blockoffset_offset, short[] block/*align 16*/, int block_offset, int stride, int[] nnzc) {
        ...
        for (i = 0; i < 16; i += 4) {
            ...
            if (nnz != 0) {
                if (nnz == 1 && block[i * 16] != 0) {
                    h264_idct8_dc_add(dst, dst_offset + blockoffset[blockoffset_offset + i], block, block_offset + i * 16, stride);
                } else {
                    h264_idct8_add(dst, dst_offset + blockoffset[blockoffset_offset + i], block, block_offset + i * 16, stride);
                }
            }
        }
